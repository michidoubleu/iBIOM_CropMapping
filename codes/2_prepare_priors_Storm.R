...
extraction <- terra::extract(prior.rast, terra::vect(pts))

extraction$CELLCODE <- temp.EEA$CELLCODE
extraction$ID <- NULL

extraction <- data.table::as.data.table(extraction)
...
#####Necessary for target matching later
